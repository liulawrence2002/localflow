...
export function LocalFlowOverlay({
  ...
  const level = clampUnit(state.level ?? defaultLevelForPhase(phase));
  const pitch = clampUnit(state.pitch ?? defaultPitchForPhase(phase));
  const brightness = clampUnit(state.brightness ?? defaultBrightnessForPhase(phase));
...
function phaseAccentColor(phase: LocalFlowDictationPhase): string {
  if (phase === "inserted") {
    return "42, 164, 103";
  }
